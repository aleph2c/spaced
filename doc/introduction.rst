...
This `spacing effect <https://en.wikipedia.org/wiki/Spacing_effect>`_ was discovered by the German psychologist `Hermann Ebbinghaus <https://en.wikipedia.org/wiki/Hermann_Ebbinghaus>`_ in the 1880s.

The algorithm used by ``spaced``, is a mash-up of a spaced-learning-algorithm with a `simple control system <https://en.wikipedia.org/wiki/PID_controller>`_ and a lightweight machine learning routine.  It provides training date suggestions, which become less frequent over time while being responsive to what a student actually does.  

You can use the results of one training session to feed in better expectations about how a student remembers and how they forget for their next session.  In this way, the ``spaced`` schedules are adaptive and will become more useful for the student as they interact with the system.  So, ``spaced`` helps the student learn, and it learns from the student as they engage with their education.
